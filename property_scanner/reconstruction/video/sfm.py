...
from .models import VideoConfig, SparseReconstruction, SparsePoint, RegisteredFrame, Observation
...
class ColmapReconstructor(SfMReconstructor):
    def reconstruct(self, images: Path, output: Path, config: VideoConfig) -> SparseReconstruction:
        ...
        matching = help_text('sequential_matcher')
        ...
        args = ['--default_random_seed','0','--database_path',database,'--image_path',str(images),'--ImageReader.single_camera','1','--ImageReader.camera_model','PINHOLE',f'--{ep}.use_gpu','0',f'--{ep}.num_threads',str(config.num_threads),'--SiftExtraction.max_num_features',str(config.colmap_max_features)]
        ...
        run('sequential_matcher',['--default_random_seed','0','--database_path',database,f'--{mp}.use_gpu','0',f'--{mp}.num_threads',str(config.num_threads),'--SequentialMatching.overlap',str(config.sequential_overlap),'--SequentialMatching.loop_detection','0'])
        sparse.mkdir(exist_ok=True)
        mapper=['--default_random_seed','0','--database_path',database,'--image_path',str(images),'--output_path',str(sparse),'--Mapper.num_threads',str(config.num_threads),'--Mapper.random_seed','0']
        ...
        result.statistics.update(components=len(models), camera_calibration='supplied' if config.intrinsics else 'COLMAP estimated', pose_convention='camera_to_world', coordinate_system='SfM arbitrary orientation and scale')
